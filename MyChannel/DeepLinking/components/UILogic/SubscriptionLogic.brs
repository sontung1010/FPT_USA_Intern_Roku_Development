    ...
    if purchases.GetChildCount() > 0
        ...
        for each purchase in allPurchases
            ...
            if utimeExpire > utimeNow
                ...
                ' if user already has expired trial subscription then we need to show catalog without trial option
                if purchase.trialQuantity > 0
...
sub OnGetCatalog(event as Object)
    ...
    if catalog.GetChildCount() > 0
        ...
        dialog.title = "Error"
        dialog.message = "There are no any available subscriptions for now..."
    end if
